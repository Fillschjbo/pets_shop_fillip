import{a}from"./read-BZCrnDQD.js";import{b as i,c as l,h as d}from"./header-vZF85M1c.js";async function c(){const t=localStorage.getItem("listingId");if(!t){alert("No postId found");return}try{return await fetch(`${i}/${t}`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("token")}`,"X-Noroff-API-Key":l}})}catch(e){console.error("error updating post: ",e)}}async function p(t){confirm("are you sure you want to delete this post?")&&(await c()?(window.location.href="/",alert("Deleted post")):alert("Failed to delete post."))}const x=document.querySelector(".listingContainer");async function m(){try{const t=await a();if(!t){alert("could not fetch data");return}const e=t.data;if(x.innerHTML=`
